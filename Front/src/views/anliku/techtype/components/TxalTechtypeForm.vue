        ...
        <a-col :span="24" v-if="!rootNode">
          <a-form-item label="父级节点" v-bind="validateInfos.pid">
            <j-tree-select placeholder="请选择父级节点" v-model:value="formData.pid" dict="txal_techtype,code,id"
              pidField="pid" pidValue="0" hasChildField="has_child" :disabled="true">
            </j-tree-select>
          </a-form-item>
        </a-col>
        <a-col :span="24">
          <a-form-item :label="`属性类型${!rootNode &&'(子层)'||'(主层)'}`" v-bind="validateInfos.sxmc">
            <a-input v-model:value="formData.sxmc" placeholder="请输入属性类型" :disabled="disabled"></a-input>
          </a-form-item>
        </a-col>
        <a-col :span="24">
          <a-form-item :label="`属性编码${!rootNode && '(子层后)'||'(主层)'}`" v-bind="validateInfos.code">
            <a-input v-model:value="formData.code" placeholder="请输入属性编码" :disabled="disabled"></a-input>
          </a-form-item>
        </a-col>
        <template v-if="!rootNode">
          <a-col :span="24">
            <a-form-item label="子层后类型名称" v-bind="validateInfos.name">
              <a-input v-model:value="formData.name" placeholder="请输入子层后类型名称" :disabled="disabled"></a-input>
            </a-form-item>
          </a-col>
          <a-col :span="24">
...
const rootNode = ref<boolean>(true);
...
const validatorRules = {
  ...
  name: [{ required: !rootNode.value, message: '请输入子层后类型名称!' }],
